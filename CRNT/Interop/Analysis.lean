...
(`δ`), `analyze_stoichRank_eq` (`s`), and `analyze_numLinkageClasses_eq` (`ℓ`). The `weaklyReversible`
flag is `decide N.WeaklyReversible`, sound by the `Decidable` instance.
