...
structure DependentReactionReturnPersistenceData
  ...
  persistence : PersistentReturnOrbitData (Concentration S)
  /-- The IFT is centered at zero rate for the newly appended reaction. -/
  parameter_zero : persistence.persistence.parameter = 0
  /-- The section vector field is exactly the smooth dependent-reaction family. -/
  field_eq : ∀ ε,
    (persistence.section ε).field = N.addedReactionFieldFamily κ q ε
