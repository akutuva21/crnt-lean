...
/-- **Differential form of the Deficiency One Theorem.** Under the three classical
structural hypotheses, the Jacobian at any positive steady state is nonsingular on
...
/-- **Nonsingularity in determinant form.**  The Jacobian on the stoichiometric tangent space
has nonzero determinant at every positive steady state.  This is the form the degree layer
consumes: it is exactly the regularity hypothesis of
`CRNT.Multistationarity.ParametrizedLocalDegree.localDegreeOn_homotopy_invariant` for the
mass-action slice of a continuation. -/
